...
function MedicareDataSource() {
  ...
  var that = this;
  $.get('/libs/datastore.json' +datasource, function(data) {
		that.setStores(that.parseJson(data));
...
MedicareDataSource.prototype.parseJson = function(json) {
	var stores=[];
    for(i=0;i<json.length;i++) {
    	var position = new google.maps.LatLng(json[i].latitude,json[i].longitude);
        var storeName = json[i].storeName;
