...
var a =$("#store-"+store.getId()).find('.locaterIcon').text();
    marker.set('labelContent', a);
...
var key = store.getId();
    ...
    var marker = new MarkerWithLabel({
       ...
       draggable: false,
       raiseOnDrag: false,
        icon:"/apps/mp/components/content/storelocator/apps-storelocator/images/balloonDisable.png",
       labelContent: a,
    ...
    if(this.get('selectedStore') && store.getId()==this.get('selectedStore').getId()) {
		marker.icon="/apps/mp/components/content/storelocator/apps-storelocator/images/balloonEnable.png";
    }
    var that=this;
	marker.clickListener_ = google.maps.event.addListener(marker, 'click',
      function() {
...
storeLocator.View.prototype.getMarker = function(store) {
  ...
  var key = store.getId();
    ...
    if( !cache[key]) {
		//cache[key].setMap(null);
        cache[key] = this['createMarker'](store);

    }
//	this.markerCache_[key].labelContent = a;
	if(this.get('selectedStore') && store.getId()==this.get('selectedStore').getId()) {
		cache[key].set("icon","/apps/mp/components/content/storelocator/apps-storelocator/images/balloonEnable.png");
    }
    else{
		cache[key].set("icon","/apps/mp/components/content/storelocator/apps-storelocator/images/balloonDisable.png");
    }
   return cache[key];
...
storeLocator.View.prototype.getInfoWindow = function(store) {
  ...
  var div = $(store.getInfoWindowContent(store.getId()));
  //  var newDiv = div[0].insertAdjacentHTML('beforeend', '<div id="two">two</div>');
...
storeLocator.View.prototype.highlight = function(store, pan) {
  ...
  if (store) {
    ...
    infoWindow.close();
	  for (var marker in this.markerCache_) {
		this.markerCache_[marker].set('icon','/apps/mp/components/content/storelocator/apps-storelocator/images/balloonDisable.png');
  }
  }
