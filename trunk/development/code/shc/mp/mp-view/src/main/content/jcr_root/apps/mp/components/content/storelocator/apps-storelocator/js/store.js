 ...
 */
storeLocator.Store.prototype.getInfoWindowContent = function(storeNo) {
  if (!this.content_) {
    var fields = ['storeName', 'address', 'email', 'phonenumber'];
    var html = ['<div class="store">'];
    html.push('<span class="locaterIcon">'+storeNo+'</span>');
    html.push('<div class="storeWrap">')
 ...
 * @return {string} a HTML version of this store.
 */
storeLocator.Store.prototype.getInfoPanelContent = function(storeNo) {
  return this.getInfoWindowContent(storeNo);
};
 ...
 */
storeLocator.Store.prototype.getInfoPanelItem = function(storeNo) {
  var cache = storeLocator.Store.infoPanelCache_;
  var store = this;
  var key = store.getId();
  if (!cache[key]) {
    var content = store.getInfoWindowContent(storeNo);
...
storeLocator.Store.prototype.distanceTo = function(point) {
  ...
  var lon2 = 0;	  
  if(point != undefined){
	  lat2 = storeLocator.toRad_(point.lat());
	  lon2 = storeLocator.toRad_(point.lng());
  }
  var dLat = lat2 - lat1;
  var dLon = lon2 - lon1;
